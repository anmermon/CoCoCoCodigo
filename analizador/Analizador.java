...
public class Analizador {
    ...
    void error(String s){ throw new Error("cerca de la linea "+lex.linea+": "+s);}
    ...
    Expr bool() throws IOException {
        ...
        while(busca.etiqueta == Etiqueta.OR) {
            Token tok = busca; mover(); x= new Or(tok,x, igualdad());
        }
        return x;
    }
    Expr unir() throws IOException{
        ...
        while( busca.etiqueta == Etiqueta.AND){
            Token tok = busca; mover(); x = new And(tok, x, rel());
        }
        return x;
    }
